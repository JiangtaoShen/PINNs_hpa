...
def func(x, t):
    # Analytical solution: u(x,t) = x * cos(t)
...
num_y = 51
test_size = 50
# Klein-Gordon domain: x ∈ [-1,1], t ∈ [0,10]
...
for epoch in tqdm(range(epochs)):
    def closure():
        ...
        # Klein-Gordon equation requires second derivatives
        u_x = torch.autograd.grad(pred_points, points_x, grad_outputs=torch.ones_like(pred_points), retain_graph=True,
                                  create_graph=True)[0]
        u_xx = \
        torch.autograd.grad(u_x, points_x, grad_outputs=torch.ones_like(u_x), retain_graph=True, create_graph=True)[0]

        u_t = torch.autograd.grad(pred_points, points_t, grad_outputs=torch.ones_like(pred_points), retain_graph=True,
                                  create_graph=True)[0]
        u_tt = \
        torch.autograd.grad(u_t, points_t, grad_outputs=torch.ones_like(u_t), retain_graph=True, create_graph=True)[0]

        # Klein-Gordon PDE: u_tt + alpha*u_xx + beta*u + gamma*u^k = -x*cos(t) + x^2*cos^2(t)
        source_term = -points_x * torch.cos(points_t) + (points_x ** 2) * (torch.cos(points_t) ** 2)
        loss_pde = torch.mean(
            (u_tt + alpha * u_xx + beta * pred_points + gamma * (pred_points ** k) - source_term) ** 2)

        # Boundary conditions: u(-1,t) = -cos(t), u(1,t) = cos(t)
        # b1 corresponds to x = -1, b2 corresponds to x = 1
        true_b1 = -torch.cos(b1_t)  # u(-1,t) = -cos(t)
        true_b2 = torch.cos(b2_t)  # u(1,t) = cos(t)
        loss_bc = torch.mean((pred_b1 - true_b1) ** 2) + torch.mean((pred_b2 - true_b2) ** 2)

        # Initial conditions: u(x,0) = x
        # b3 corresponds to t = 0
        true_b3 = b3_x  # u(x,0) = x
        loss_ic1 = torch.mean((pred_b3 - true_b3) ** 2)

        # Initial condition: du/dt(x,0) = 0
        # Need to compute u_t at t=0 (b3 points)
        pred_b3_ut = \
        torch.autograd.grad(pred_b3, b3_t, grad_outputs=torch.ones_like(pred_b3), retain_graph=True, create_graph=True)[
            0]
        loss_ic2 = torch.mean(pred_b3_ut ** 2)  # du/dt(x,0) = 0

        loss_ic = loss_ic1 + loss_ic2

        # L2 metric against analytical solution
        true_test = func(points_test_x, points_test_t)[:, 0:1, :]
...
print(
    f" Losses - PDE: {final_loss_dict['pde_loss']:.4e} | "f"BC: {final_loss_dict['bc_loss']:.4e} | "f"IC: {final_loss_dict['ic_loss']:.4e} | "f"Total: {final_loss_dict['total_loss']:.4e} | "f"l2: {final_loss_dict['l2_metric']:.4e}")

with open('./output/m7_pde6.pkl', 'wb') as f:
    pickle.dump({
...
plt.pcolormesh(points_test_x[:, 0:1, :].cpu().detach().numpy().reshape(test_size, test_size),
               points_test_t[:, 0:1, :].cpu().detach().numpy().reshape(test_size, test_size), u_pred, cmap='rainbow',
               shading='gouraud')
...
plt.tight_layout()
plt.savefig('./output/m7_pde6_predicted.pdf')
plt.show()

# Plot 2: True physical field
plt.figure(figsize=(5, 4))
plt.pcolormesh(points_test_x[:, 0:1, :].cpu().detach().numpy().reshape(test_size, test_size),
               points_test_t[:, 0:1, :].cpu().detach().numpy().reshape(test_size, test_size), u_true, cmap='rainbow',
               shading='gouraud')
...
plt.ylim(range_x[1].cpu().numpy()[0], range_x[1].cpu().numpy()[1])
plt.tight_layout()
plt.savefig('./output/m7_pde6_ground_truth.pdf')
plt.show()

# Plot 3: Error
plt.figure(figsize=(5, 4))
plt.pcolormesh(points_test_x[:, 0:1, :].cpu().detach().numpy().reshape(test_size, test_size),
               points_test_t[:, 0:1, :].cpu().detach().numpy().reshape(test_size, test_size), np.abs(u_true - u_pred),
...
plt.ylim(range_x[1].cpu().numpy()[0], range_x[1].cpu().numpy()[1])
plt.tight_layout()
plt.savefig('./output/m7_pde6_error.pdf')
plt.show()