...
# PDE3 的解析解
def func(x, t):
    spatial = torch.sin(np.pi * x)
    temporal = torch.exp(-t)
...
for epoch in tqdm(range(epochs)):
    def closure():
        ...
        # PDE3 残差: u_t - u_xx + exp(-t) * (sin(πx) - π²sin(πx)) = 0
        ...
        source_term = torch.exp(-points_t) * (
                torch.sin(np.pi * points_x) - (np.pi ** 2) * torch.sin(np.pi * points_x))

        loss_pde = torch.mean((u_t - u_xx + source_term) ** 2)

        # 边界条件: u(0,t) = 0 和 u(2π,t) = 0
...
with open('./output/m7_pde3.pkl', 'wb') as f:
    pickle.dump({
...
u_true = func(points_test_x, points_test_t)[:, 0:1,:]
...
plt.tight_layout()
plt.savefig('./output/m7_pde3_predicted.pdf')
plt.show()
...
plt.ylim(range_x[1].cpu().numpy()[0], range_x[1].cpu().numpy()[1])
plt.tight_layout()
plt.savefig('./output/m7_pde3_ground_truth.pdf')
plt.show()
...
plt.ylim(range_x[1].cpu().numpy()[0], range_x[1].cpu().numpy()[1])
plt.tight_layout()
plt.savefig('./output/m7_pde3_error.pdf')
plt.show()