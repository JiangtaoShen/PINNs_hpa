...
def func(x, t):
    hx = h(x)
    return (hx * torch.exp(rho * t)) / (hx * torch.exp(rho * t) + 1 - hx)
...
for epoch in tqdm(range(epochs)):
    def closure():
        ...
        u_t = torch.autograd.grad(
            pred_points, points_t,
            grad_outputs=torch.ones_like(pred_points),
            retain_graph=True, create_graph=True
        )[0]

        # PDE residual: u_t - rho*u*(1-u) = 0
        loss_pde = torch.mean((u_t - rho*pred_points*(1 - pred_points)) ** 2)

        # boundary condition: u(0,t) = u(2π,t)
...
with open('./output/m7_pde2.pkl', 'wb') as f:
    pickle.dump({
...
plt.tight_layout()
plt.savefig('./output/m7_pde2_predicted.pdf')
plt.show()
...
plt.ylim(range_x[1].cpu().numpy()[0], range_x[1].cpu().numpy()[1])
plt.tight_layout()
plt.savefig('./output/m7_pde2_ground_truth.pdf')
plt.show()
...
plt.ylim(range_x[1].cpu().numpy()[0], range_x[1].cpu().numpy()[1])
plt.tight_layout()
plt.savefig('./output/m7_pde2_error.pdf')
plt.show()