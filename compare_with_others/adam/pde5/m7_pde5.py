...
t_data, x_data, usol = data['t'], data['x'], data['usol']
data.close()
...
true_test = usol_sub.T.ravel()
true_test = torch.from_numpy(true_test).float().to(device).unsqueeze(1)
points_test_origin = torch.from_numpy(points_test).float().to(device)
...
for epoch in tqdm(range(epochs)):
    def closure():
        ...
        # PDE5 Burgers: u_t + u u_x = nu u_xx
        ...
        loss_pde = torch.mean((u_t + pred_points * u_x - nu * u_xx) ** 2)
...
with open('./output/m7_pde5.pkl', 'wb') as f:
    pickle.dump({
...
# ----------------------------
with torch.no_grad():
    u_pred = model(points_test_x, points_test_x)[:, 0:1,:]
...
plt.tight_layout()
plt.savefig('./output/m7_pde5_predicted.pdf')
plt.show()
...
plt.ylabel('t', fontsize=15)
plt.tight_layout()
plt.savefig('./output/m7_pde5_ground_truth.pdf')
plt.show()
...
plt.ylabel('t', fontsize=15)
plt.tight_layout()
plt.savefig('./output/m7_pde5_error.pdf')
plt.show()