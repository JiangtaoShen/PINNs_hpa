...
# PDE4 的解析解
def func(x, t):
    term1 = torch.sin(np.pi * x) * torch.cos(2 * np.pi * t)
    term2 = 0.5 * torch.sin(beta * np.pi * x) * torch.cos(2 * beta * np.pi * t)
    return term1 + term2
...
num_y = 51
test_size = 50
range_x = torch.tensor([[0., 1.], [0., 1.]]).to(device)  # 修改为 x ∈ [0,1], t ∈ [0,1]
...
for epoch in tqdm(range(epochs)):
    def closure():
        ...
        loss_pde = torch.mean((u_tt - alpha * u_xx) ** 2)

        # 边界条件: u(0,t) = 0 和 u(1,t) = 0
        pred_b1 = model(b1_x, b1_t)
        pred_b2 = model(b2_x, b2_t)
        loss_bc = torch.mean((pred_b1) ** 2) + torch.mean((pred_b2) ** 2)

        # 初始条件1: u(x,0) = sin(πx) + 0.5*sin(βπx)
        pred_b3 = model(b3_x, b3_t)
        true_b3 = torch.sin(np.pi * b3_x) + 0.5 * torch.sin(beta * np.pi * b3_x)
        loss_ic1 = torch.mean((pred_b3 - true_b3) ** 2)

        # 初始条件2: u_t(x,0) = 0
        # 计算在初始时刻的时间导数
        pred_b3_t = torch.autograd.grad(pred_b3, b3_t,
                                        grad_outputs=torch.ones_like(pred_b3),
                                        retain_graph=True, create_graph=True)[0]
        loss_ic2 = torch.mean((pred_b3_t) ** 2)
        ...
        true_test = true_test.squeeze(-1)
        l2_metric = torch.norm(pred_test - true_test) / torch.norm(true_test)

        loss_total = loss_pde + loss_bc + loss_ic1 + loss_ic2

        train_loss_dict = {
            'pde_loss': loss_pde.item(),
            'bc_loss': loss_bc.item(),
            'ic1_loss': loss_ic1.item(),
            'ic2_loss': loss_ic2.item(),
...
print(f" Losses - PDE: {final_loss_dict['pde_loss']:.4e} | "
      ...
      f"IC1: {final_loss_dict['ic1_loss']:.4e} | "
      f"IC2: {final_loss_dict['ic2_loss']:.4e} | "
      f"Total: {final_loss_dict['total_loss']:.4e} | "
      f"L2: {final_loss_dict['l2_metric']:.4e}")

with open('./output/m7_pde4.pkl', 'wb') as f:
    pickle.dump({
...
plt.tight_layout()
plt.savefig('./output/m7_pde4_predicted.pdf')
plt.show()
...
plt.ylim(range_x[1].cpu().numpy()[0], range_x[1].cpu().numpy()[1])
plt.tight_layout()
plt.savefig('./output/m7_pde4_ground_truth.pdf')
plt.show()
...
plt.ylim(range_x[1].cpu().numpy()[0], range_x[1].cpu().numpy()[1])
plt.tight_layout()
plt.savefig('./output/m7_pde4_error.pdf')
plt.show()