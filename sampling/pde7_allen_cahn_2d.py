...
def allen_cahn_2d(num_layers, num_nodes, activation_func, epochs, grid_size, learning_rate, test_name, plotshow = False, device='cpu'):
    ...
    def losses(model, points, b1, b2, b3, b4, points_test, lam_pde=1.0, lam_bc=1.0, lam_ic=1.0):
        ...
        # pred_b3 = model(b3)
        # true_b3 = b3[:, 0:1]**2 * torch.cos(np.pi * b3[:, 0:1])
