...
def kdv_2d(num_layers, num_nodes, activation_func, epochs, grid_size, learning_rate, test_name, plotshow = False, device='cpu'):
    ...
    # PDEs
    # PDE for KdV Equation: du/dt + k*u*du/dx + d3u/dx3 = 0
    def pde(x, y):
        ...
        # Compute third derivative ∂³u/∂x³
        dy_xxx = torch.autograd.grad(dy_xx, x, torch.ones_like(dy_xx), create_graph=True)[0][:, 0:1]
        k = 6
        return dy_t + k * y * dy_x + dy_xxx

    # Analytical solution: u(x,t) = 2*sech²(x - 4t)
    def func(x):
        # sech(z) = 1/cosh(z) = 2/(exp(z) + exp(-z))
        z = x[:, 0:1] - 4 * x[:, 1:2]  # x - 4t
    ...
    def losses(model, points, b1, b2, b3, b4, points_test, lam_pde=1.0, lam_bc=1.0, lam_ic=1.0):
        ...
        # Initial condition: u(x,0) = 2*sech²(x)
        pred_b3 = model(b3)  # t=0
        true_b3 = func(b3)  # u(x,0) = 2*sech²(x)
    ...
    try:
        ...
        num_x_test = 100
        num_y_test = 100
        range_x = torch.tensor([[-10., 10.], [0., 1.]]).to(device)
