...
def heat_2d(num_layers, num_nodes, activation_func, epochs, grid_size, learning_rate, test_name, plotshow = False, device='cpu'):
    ...
    # Analytical solution: u(x,t) = exp(-0.4*n²*π²*t) * sin(n*π*x)
    def func(x):
        n = 15
        pi = torch.pi
        return torch.exp(-alpha * n ** 2 * pi ** 2 * x[:, 1:2]) * torch.sin(n * pi * x[:, 0:1])
    ...
    def losses(model, points, b1, b2, b3, b4, points_test, lam_pde=1.0, lam_bc=1.0, lam_ic=1.0):
        ...
        pred_b1 = model(b1)  # x=0
        true_b1 = torch.zeros_like(pred_b1)  # u(0,t) = 0
        pred_b2 = model(b2)  # x=1
        true_b2 = torch.zeros_like(pred_b2)  # u(1,t) = 0
        bc_loss = torch.mean((pred_b1 - true_b1) ** 2) + torch.mean((pred_b2 - true_b2) ** 2)

        # Initial condition: u(x,0) = sin(n*π*x)
        pred_b3 = model(b3)  # t=0
        true_b3 = func(b3)  # u(x,0) = sin(n*π*x)
...
# heat_2d(num_layers=3, num_nodes=30, activation_func='tanh', epochs=50000, grid_size=40, learning_rate=0.001, test_name='test', plotshow = True, device='cuda')
