...
def ad_2d(num_layers, num_nodes, activation_func, epochs, grid_size, learning_rate, test_name, plotshow = False, device='cpu'):
    ...
    def pde(x, y):
        ...
        diffusion_coeff = 0.1
        return dy_t + dy_x - diffusion_coeff * dy_xx

    # Analytical solution: u(x,t) = sqrt(0.25/(0.25+0.1*t)) * exp(-(x-t)^2/(1+0.4*t))
    def func(x):
        t = x[:, 1:2]  # time
        x_coord = x[:, 0:1]  # space coordinate

        # Time-dependent variance
        variance_factor = 0.25 + 0.1 * t
        denominator = 1.0 + 0.4 * t

        # Amplitude factor
        amplitude = torch.sqrt(0.25 / variance_factor)

        # Exponential term
        exponent = -(x_coord - t) ** 2 / denominator
    ...
    def losses(model, points, b1, b2, b3, b4, points_test, lam_pde=1.0, lam_bc=1.0, lam_ic=1.0):
        ...
        # Boundary conditions: u(-10,t) = 0 and u(10,t) = 0
        pred_b1 = model(b1)  # x=-10
        true_b1 = torch.zeros_like(pred_b1)  # u(-10,t) = 0
        pred_b2 = model(b2)  # x=10
        true_b2 = torch.zeros_like(pred_b2)  # u(10,t) = 0
        bc_loss = torch.mean((pred_b1 - true_b1) ** 2) + torch.mean((pred_b2 - true_b2) ** 2)

        # Initial condition: u(x,0) = exp(-x^2)
        pred_b3 = model(b3)  # t=0
        true_b3 = torch.exp(-b3[:, 0:1] ** 2)  # u(x,0) = exp(-x^2)
    ...
    try:
        ...
        num_x_test = 100
        num_y_test = 100
        range_x = torch.tensor([[-5., 15.], [0., 10.]]).to(device)
...
# ad_2d(num_layers=3, num_nodes=40, activation_func='tanh', epochs=50000, grid_size=50, learning_rate=0.0005, test_name='test', plotshow = True, device='cuda')